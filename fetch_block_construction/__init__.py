        ...
        for obs_type in ['dictimage', 'np', 'dictstate']:
            for render_size in [42, 84]:
                for stack_only in [True, False]:
                    for case in ["Singletower", "Pyramid", "Multitower", "All","PickAndPlace"]:
                        initial_qpos = {
                            'robot0:slide0': 0.405,
                            'robot0:slide1': 0.48,
                            'robot0:slide2': 0.0,
            ...
            for render_size in [42, 84]:
                for stack_only in [True, False]:
                    for case in ["PickAndDrop","PickAndPlace","Reach"]:
                        ...
                        register(
                            id='FetchBlockHRL_{}Blocks_{}Reward_{}Obs_{}Rendersize_{}Stackonly_{}Case-v1'.format(*[kwarg.title() if isinstance(kwarg, str) else kwarg for kwarg in [num_blocks, reward_type, obs_type, render_size, stack_only, case]]),
                            entry_point='fetch_block_construction.envs.robotics:FetchBlockHRLEnv',
                            kwargs=kwargs,
                            max_episode_steps=50 * 1,
                        )